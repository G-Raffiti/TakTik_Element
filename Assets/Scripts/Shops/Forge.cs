...
namespace Shops
{
    public class Forge : MonoBehaviour
    {
        ...
        {
            Gear gear = new Gear();
            ForgeUI forgeUI = GameObject.Find("ForgeUI/Left/Main").GetComponent<ForgeUI>();
            if (forgeUI.GetCraftStats() == null)
                return;

            foreach (KeyValuePair<AffixSO,int> _pair in forgeUI.GetCraftStats())
            {
            ...
            gear.CraftNewGear(forgeUI.GetCraftStats());
            
            forgeUI.ShowCraftedGear(gear);
            onDiplayUptade.Raise();
        }
        ...
        {
            ForgeUI forgeUI = GameObject.Find("ForgeUI/Left/Main").GetComponent<ForgeUI>();
            Debug.Log("1 Forge founded :" + (forgeUI != null));
            UpgradeGear(gear, forgeUI.GetUpgradeAffix());
            onDiplayUptade.Raise();
        }
